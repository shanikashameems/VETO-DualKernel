...
import { Terminal, ShieldAlert, ShieldCheck, CheckCircle2, AlertOctagon, ArrowRight } from 'lucide-react';
...
export const DualKernelTrace: React.FC<DualKernelTraceProps> = ({
  ...
  const executionVetoMode = dispatchResult ? dispatchResult.veto_mode : vetoMode;
  ...
  return (
    <div className="bg-[#141720] border border-gray-800 rounded-lg flex flex-col h-full shadow-xl overflow-hidden">
      ...
      {/* TERMINAL BODY */}
      <div className="flex-1 p-4 font-mono text-xs overflow-y-auto custom-scrollbar space-y-5 bg-[#0F1117]">
        ...
        {candidates && visibleLogs.some(l => l.stage === 'STAGE 1' && l.message.includes('emitted')) && (
          <div className="bg-[#1A1E2E] border border-blue-900/60 rounded p-3 text-xs space-y-1.5 my-2">
            <div className="text-[11px] font-bold text-blue-400 uppercase tracking-wider mb-1">
              [KERNEL 01] PROPOSED CANDIDATE PARAMETERS
            </div>
            <div className="grid grid-cols-2 gap-2 text-gray-300">
              ...
              <div><span className="text-gray-500">amount:</span> <span className="text-emerald-400 font-bold">₹{candidates.amount.toLocaleString()}</span></div>
              <div>
        ...
        {/* LINEAGE RECONCILIATION BOX (STAGE 2) */}
        {evaluation && visibleLogs.some(l => l.stage === 'STAGE 2') && (
          <div className="bg-[#1C1520] border border-red-900/40 rounded p-3 space-y-2 my-2">
            <div className="text-[11px] font-bold text-amber-400 uppercase tracking-wider flex items-center justify-between">
              <span>[STAGE 2] CAUSAL LINEAGE & RECONCILIATION</span>
              <span className="text-gray-500 text-[10px]">SQLITE ERP TRUTH QUERY</span>
            </div>

            <div className="bg-[#11131C] p-2 rounded text-[11px] text-gray-300 font-mono">
              <p className="text-blue-300">SELECT verified_account FROM erp_vendors WHERE name = '{candidates?.vendor}';</p>
              <div className="mt-1 flex items-center justify-between text-xs">
                <span>Enterprise Registry Account: <strong className="text-emerald-400">{evaluation.expected_account}</strong></span>
                <span>Derived Invoice Account: <strong className={isTainted ? 'text-red-400' : 'text-emerald-400'}>{evaluation.received_account}</strong></span>
              </div>
            </div>
            ...
            {isTainted ? (
              <div className="bg-red-950/80 border border-red-600 rounded p-2.5 flex items-center justify-between text-red-200">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <div className="font-bold text-xs text-red-400">TAINT STATE FLIPPED: param('beneficiary_account') = TAINTED</div>
                    <div className="text-[11px] text-red-300 font-mono">INVARIANT MISMATCH: {evaluation.received_account} != {evaluation.expected_account}</div>
                  </div>
                </div>
              ...
              <div className="bg-emerald-950/80 border border-emerald-600 rounded p-2.5 flex items-center justify-between text-emerald-200">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <div className="font-bold text-xs text-emerald-400">PROVENANCE VERIFIED</div>
                    <div className="text-[11px] text-emerald-300 font-mono">INVARIANT MATCH: {evaluation.received_account} == {evaluation.expected_account}</div>
                  </div>
                </div>
        ...
        {evaluation && visibleLogs.some(l => l.stage === 'STAGE 3' && (l.timestamp.includes('0.70s') || l.timestamp.includes('0.62s'))) && (
              ...
              <div className="bg-emerald-950/90 border-2 border-emerald-500 rounded-lg p-4 text-emerald-100 shadow-lg space-y-2">
                <div className="flex items-center justify-between border-b border-emerald-800/80 pb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-bold text-sm text-emerald-300 font-mono tracking-wide">
                      SYSTEM INVARIANT UPHELD — ZERO FUNDS MOVED
                    </span>
                  </div>
                ...
                <div className="text-xs font-mono space-y-1 text-emerald-200/90">
                  <p>✓ VETO Gateway intercepted tainted parameters prior to egress.</p>
                  <p>✓ Local Mock Banking API was NEVER called.</p>
              ...
              <div className="bg-red-950/90 border-2 border-red-500 rounded-lg p-4 text-red-100 shadow-lg space-y-2 animate-bounce-once">
                <div className="flex items-center justify-between border-b border-red-800/80 pb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-bold text-sm text-red-200 font-mono tracking-wide">
              ...
              <div className="bg-blue-950/90 border-2 border-blue-500 rounded-lg p-4 text-blue-100 shadow-lg space-y-2">
                <div className="flex items-center justify-between border-b border-blue-800/80 pb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-bold text-sm text-blue-200 font-mono tracking-wide">
                ...
                <div className="text-xs font-mono space-y-1 text-blue-200">
                  ...
                  <p>✓ Mock payment of ₹5,00,000 settled successfully.</p>
                </div>
              </div>
            )}
          </div>
